...
import {
    Image, View, Text, Pressable, TextInput, StyleSheet, ScrollView, FlatList
} from 'react-native'
...
const AddSiteScreen = ({ route }) => {
    ...
    const getLiveLocation = async (lat, lng) => {
        ...
        if (locPermissionDenied) {
            const url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=" + lat + "," + lng + "&key=AIzaSyAEKIGD-7QbMF1ZEtd7PVabDq_yKx5eyIc";
            try {
    ...
    const add_site = () => {
        if (!SiteDescription || !SiteName) {
            ...
            create_sites(formdata)
                .then(result => {
                    if (result?.status == "1") {
                        navigation.navigate('Preview', { tourID: data?.tours_id, tourName, price })
                    }
                })
        ...
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <View
                ...
                <Pressable
                    ...
                    style={{ width: 35, height: 35, position: 'absolute', left: 15, top: 15 }}>
                    <Image
            ...
            <ScrollView
                ...
                <View style={{ padding: 15 }}>
                    ...
                    <TextInput
                        style={[styles.inputStyl, { minHeight: 130, textAlignVertical: 'top', paddingTop: 20 }]}
                    ...
                    <Pressable
                        ...
                        style={{ height: 130, marginTop: 15, borderRadius: 10, overflow: "hidden", backgroundColor: "#F8F8F8", justifyContent: "center", marginBottom: 100 }}>
                        {image?.length === 0 ?
                            <Image
                        ...
                        {image?.length === 0 ? <Text style={styles.txtStyl}>Audios, Photos, Videos of the Sites</Text> : null}
                    </Pressable>
            ...
            <MyButton
                ...
                style={{ borderRadius: 30, width: "90%", alignSelf: "center", height: 55, marginTop: 35, marginBottom: 10 }}
            />
            <MyButton
                title={"Finish Tour"}
                loading={loading}
                onPress={add_site}
                textStyle={{ fontSize: 18, fontFamily: Theme.FONT_FAMILY_SEMIBOLD, lineHeight: 30, }}
                style={{ borderRadius: 30, width: "90%", alignSelf: "center", height: 55, marginTop: 10, marginBottom: 20 }}
            />
            <UploadImageModal
                isVisible={show}
