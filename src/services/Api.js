...
export const create_tour = async postdata => {
  ...
  const URL = DOMAIN + 'uploads/tours-add';
  return authAxios.post(URL, postdata, {
    headers: headers,
  });
};
...
export const delete_sites = async postdata => {
  const URL = DOMAIN + 'sites/delete';
  return authAxios.post(URL, postdata);
};

export const create_sites = async postdata => {
  const headers = { 'Content-Type': 'multipart/form-data' };
  const URL = DOMAIN + 'uploads/sites-add';
  return authAxios.post(URL, postdata, {
    headers: headers,
  });
};
...
export const get_inbox = async postdata => {
  const URL = DOMAIN + `chats/all-recent-messages/${postdata}`;
  return authAxios.get(URL);
};

export const delete_account = async postdata => {
  const URL = DOMAIN + `user/delete/${postdata?.user_id}?password=${postdata?.password}`;
  return authAxios.get(URL);
};


export const delete_tourImage = async postdata => {
  const URL = DOMAIN + `tours/delete/gallery/${postdata}`;
  return authAxios.post(URL);
};
